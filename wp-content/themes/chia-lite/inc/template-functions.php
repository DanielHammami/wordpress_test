 ...
 * @return array
 */
function chia_lite_body_classes( $classes ) {
	...
	if ( 'fixed-header' === get_theme_mod( 'chia_lite_header_layout' ) ) {
		$classes[] = 'fixed-header';
	}
	if ( 'static-header' === get_theme_mod( 'chia_lite_header_layout' ) ) {
		$classes[] = 'static-header';
	}

	// Add class for the skip to top
	if ( get_theme_mod( 'chia_lite_hide_skip' ) ) {
		$classes[] = 'skip-to-top';
	}

	// Add class for the excerpt link
	if ( get_theme_mod( 'chia_lite_excerpt_link' ) ) {
		$classes[] = 'excerpt-link';
	}

	// Add class for the full text link
	if ( get_theme_mod( 'chia_lite_full_text_link' ) ) {
		$classes[] = 'full-text-link';
	}

	// Add class for the body animation
	if ( !get_theme_mod( 'chia_lite_page_loading_animation' ) ) {
		$classes[] = 'page-animation';
	}

	return $classes;
}
add_filter( 'body_class', 'chia_lite_body_classes' );
 ...
 * @return array
 */
function chia_lite_post_classes( $classes ) {
	$classes[] = 'entry';

	return $classes;
}
add_filter( 'post_class', 'chia_lite_post_classes', 10, 3 );
 ...
 * @return void
 */
function chia_lite_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}
add_action( 'wp_head', 'chia_lite_pingback_header' );
 ...
 * @return void
 */
function chia_lite_supports_js() {
	echo '<script>document.body.classList.remove("no-js");</script>';
}
add_action( 'wp_footer', 'chia_lite_supports_js' );
 ...
 * @return array
 */
function chia_lite_comment_form_defaults( $defaults ) {
...
add_filter( 'comment_form_defaults', 'chia_lite_comment_form_defaults' );
 ...
 * @return bool
 */
function chia_lite_can_show_post_thumbnail() {
 ...
 * @return int
 */
function chia_lite_get_avatar_size() {
	return 60;
}
 ...
 */
function chia_lite_continue_reading_text() {
	$continue_reading = sprintf(
		/* translators: %s: Name of current post. */
		esc_html__( 'Continue reading %s', 'chia-lite' ),
 ...
 */
function chia_lite_continue_reading_link() {
	if ( ! is_admin() ) {
		return '<div class="more-link-container"><a class="more-link" href="' . esc_url( get_permalink() ) . '#more-' . esc_attr( get_the_ID() ) . '">' . chia_lite_continue_reading_text() . '</a></div>';
	}
}

add_filter( 'the_content_more_link', 'chia_lite_continue_reading_link' );

if ( ! function_exists( 'chia_lite_post_title' ) ) {
	/**
	 ...
	 * @return string
	 */
	function chia_lite_post_title( $title ) {
		return '' === $title ? esc_html_x( 'Untitled', 'Added to posts and pages that are missing titles', 'chia-lite' ) : $title;
	}
}
add_filter( 'the_title', 'chia_lite_post_title' );
 ...
 * @return string
 */
function chia_lite_get_icon_svg( $group, $icon, $size = 24 ) {
	return Chia_Lite_SVG_Icons::get_svg( $group, $icon, $size );
}
 ...
 * @return string
 */
function chia_lite_change_calendar_nav_arrows( $calendar_output ) {
	$calendar_output = str_replace( '&laquo; ', is_rtl() ? chia_lite_get_icon_svg( 'ui', 'arrow_right' ) : chia_lite_get_icon_svg( 'ui', 'arrow_left' ), $calendar_output );
	$calendar_output = str_replace( ' &raquo;', is_rtl() ? chia_lite_get_icon_svg( 'ui', 'arrow_left' ) : chia_lite_get_icon_svg( 'ui', 'arrow_right' ), $calendar_output );
	return $calendar_output;
}
add_filter( 'get_calendar', 'chia_lite_change_calendar_nav_arrows' );
 ...
 * @return string
 */
function chia_lite_get_non_latin_css( $type = 'front-end' ) {
	...
	$font_family = apply_filters(
		'chia_lite_get_localized_font_family_types',
		array(
	...
	$elements = apply_filters(
		'chia_lite_get_localized_font_family_elements',
		array(
	...
	if ( ! function_exists( 'chia_lite_generate_css' ) ) {
	...
	return chia_lite_generate_css( // @phpstan-ignore-line.
 ...
 */
function chia_lite_print_first_instance_of_block( $block_name, $content = null, $instances = 1 ) {
	$instances_count = 0;
	$blocks_content  = '';
 ...
 */
function chia_lite_password_form( $post = 0 ) {
	$post   = get_post( $post );
	$label  = 'pwbox-' . ( empty( $post->ID ) ? wp_rand() : $post->ID );
	$output = '<p class="post-password-message">' . esc_html__( 'This content is password protected. Please enter a password to view.', 'chia-lite' ) . '</p>
	<form action="' . esc_url( site_url( 'wp-login.php?action=postpass', 'login_post' ) ) . '" class="post-password-form" method="post">
	<label class="post-password-form__label" for="' . esc_attr( $label ) . '">' . esc_html_x( 'Password', 'Post password form', 'chia-lite' ) . '</label><input class="post-password-form__input" name="post_password" id="' . esc_attr( $label ) . '" type="password" size="20" /><input type="submit" class="post-password-form__submit" name="' . esc_attr_x( 'Submit', 'Post password form', 'chia-lite' ) . '" value="' . esc_attr_x( 'Enter', 'Post password form', 'chia-lite' ) . '" /></form>
	';
	return $output;
}
add_filter( 'the_password_form', 'chia_lite_password_form' );
 ...
 * @return array
 */
function chia_lite_get_attachment_image_attributes( $attr, $attachment, $size ) {
...
add_filter( 'wp_get_attachment_image_attributes', 'chia_lite_get_attachment_image_attributes', 10, 3 );
