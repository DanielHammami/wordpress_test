...
if ( ! class_exists( 'Bloz_About_Us_Image_Widget' ) ) :

	/**
	 * Adds To about widget.
	 */
	class Bloz_About_Us_Image_Widget extends WP_Widget {
		...
		public function __construct() {
			$widget_ops = array(
				...
				'description' => esc_html__( 'An widget to upload About Content and images in sidebar.', 'bloz' ),
			);
			parent::__construct( 'widget_about', esc_html__('TO: About Us Widget','bloz'), $widget_ops );
		}
		...
		public function widget( $args, $instance ) {
			...
			<div class="textwidget">
				...
				?>
				<h4><?php echo esc_html( $tpiw_name ); ?></h4>
					...
					<ul class="menu">
						...
						<li>
							<a href="<?php echo esc_url( $link ) . '" ' . esc_attr( $target ); ?>">
								<?php echo bloz_return_social_icon( $link ); ?>
							</a>
						</li>
		...
		public function form( $instance ) {
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'name' ) ); ?>"><?php esc_html_e( 'Name :', 'bloz' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'description' ) ); ?>"><?php esc_html_e( 'Description :', 'bloz' ); ?></label>
			...
			<div>
				<label for="<?php echo esc_attr( $this->get_field_id( 'tpiw_image_url' ) ); ?>"><?php esc_html_e( 'Image URL', 'bloz' ); ?></label>:<br />
				...
				<input type="button" class="select-img button button-primary" value="<?php esc_attr_e( 'Upload', 'bloz' ); ?>" data-uploader_title="<?php esc_attr_e( 'Select Image', 'bloz' ); ?>" data-uploader_button_text="<?php esc_attr_e( 'Choose Image', 'bloz' ); ?>" style="margin-top:5px;" />
		      	...
		      	<div class="tpiw-preview-wrap" <?php echo esc_attr( $wrap_style ); ?>>
		        	<img src="<?php echo esc_url( $full_image_url ); ?>" alt="<?php esc_attr_e('Preview', 'bloz'); ?>" style="max-width: 100%;"/>
		      	</div><!-- .tpiw-preview-wrap -->

	    	</div>

		   <p>
			   	<label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php esc_html_e( 'Number of links to show:', 'bloz' ); ?></label>
		   ...
		   <p>
		    	<label for="<?php echo esc_attr( $this->get_field_id( 'open_link' ) ); ?>"><?php esc_html_e( 'Open Social link in New Tab', 'bloz' ); ?>:</label>
		      	<input type="checkbox" id="<?php echo esc_attr( $this->get_field_id( 'open_link' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'open_link' ), 'bloz' ); ?>"  <?php checked( $open_link, true ); ?> />
		   </p>

		   <?php for ( $i=1; $i <= $number; $i++ ) {
		   	$link = isset( $instance['link'. '-' . $i ] ) ? $instance['link' . '-' . $i ] : '';?>
			   <p>
				   	<label for="<?php echo esc_attr( $this->get_field_id( 'link' . '-' . $i ) ); ?>"><?php printf( esc_html__( 'Link %s :', 'bloz' ), $i ); ?></label>
		...
		public function update( $new_instance, $old_instance ) {
			...
			$instance['open_link']       	= $new_instance['open_link'];
 ...
 */
function bloz_about_us_image_widget_upload_enqueue( $hook ) {

  if( 'widgets.php' !== $hook )
      return;

  wp_enqueue_media();
  wp_enqueue_script( 'bloz-image-widget-upload-script', get_template_directory_uri() . '/assets/js/upload.js', array( 'jquery' ), '1.1', true );

}
add_action( 'admin_enqueue_scripts', 'bloz_about_us_image_widget_upload_enqueue' );