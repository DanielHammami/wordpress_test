...
jQuery(function($){
      ...
      jQuery(document).on('click', 'input.select-img', function( event ){
        ...
        var BlozImage = wp.media.controller.Library.extend({
            defaults :  _.defaults({
        ...
        bloz_file_frame = wp.media.frames.bloz_file_frame = wp.media({
          button: {
        ...
        bloz_file_frame.on( 'select', function() {

          var state = bloz_file_frame.state('tpiw-insert-image');
