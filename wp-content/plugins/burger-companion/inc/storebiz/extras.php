...
if ( ! function_exists( 'storebiz_header_offer' ) ) {
	function storebiz_header_offer() {
		...
		$hdr_nav_offer_icon		=	get_theme_mod('hdr_nav_offer_icon','fa-gift');
				...
				<ul>
				   <?php
					...
					foreach ( $hdr_nav_offer_content as $offer_item ) {
						$title = ! empty( $offer_item->title ) ? apply_filters( 'storebiz_translate_single_string', $offer_item->title, 'slider section' ) : '';
						$link = ! empty( $offer_item->link ) ? apply_filters( 'storebiz_translate_single_string', $offer_item->link, 'slider section' ) : '';
							
					?>
					<li><i class="fa <?php echo esc_attr($hdr_nav_offer_icon); ?>"></i> <a href="<?php echo esc_url($link); ?>"><?php echo esc_html($title); ?></a></li>
					<?php } } ?>
				</ul>
			</div>
		</div>
